...
def main() -> int:
    ...
    parser.add_argument("--provider", default="google", choices=["google", "ollama", "openrouter", "groq", "custom"])
    parser.add_argument("--model-name", default=None)
    parser.add_argument("--today", default="2026-06-01")
    parser.add_argument("--pass-threshold", type=float, default=80.0)
    parser.add_argument("--judge-provider", default=None, choices=["google", "ollama", "openrouter", "groq", "custom"])
    ...
    for case in cases:
        print(f"Scoring case {case['id']}...", file=sys.stderr)
        ...
        while retries > 0:
            try:
                ...
                if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str or "503" in err_str or "UNAVAILABLE" in err_str:
                    print(f"Rate limit or server error hit ({err_str[:40]}), sleeping for 45 seconds...", file=sys.stderr)
                    time.sleep(45)
                    retries -= 1
                    if retries == 0:
                        raise e
